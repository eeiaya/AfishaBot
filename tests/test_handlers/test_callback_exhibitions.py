...
# Импортируем модули
from app.handlers import callback_exhibitions
from utils.states import user_states


@pytest.fixture
def mock_dependencies():
    with patch("services.CollectData.CollectData.collect_data_exhibitions", new=MagicMock()) as mock_collect_data, \
...
@pytest.mark.asyncio
async def test_callback_exhibitions(mock_dependencies, mock_bot):
    ...
    with patch.dict(user_states, {}, clear=True):  # Очищаем словарь
        # Патчим переданный бот в аргументы callback_exhibitions
        with patch("app.handlers.bot", new=mock_bot):
            # Вызов тестируемого обработчика
            await callback_exhibitions(callback)

        # Проверяем, что collect_data_exhibitions была вызвана
